...
AS 
    ...
    from
                ...
                STREAM(wsuber.bronze.stg_rides)
                WATERMARK booking_timestamp DELAY OF INTERVAL 3 MINUTES stg_rides
            
        
            
                LEFT Join wsuber.bronze.map_vehicle_makes map_vehicle_makes on stg_rides.vehicle_make_id=map_vehicle_makes.vehicle_make_id
            
        
            
                LEFT Join wsuber.bronze.map_vehicle_types map_vehicle_types on stg_rides.vehicle_type_id=map_vehicle_types.vehicle_type_id
            
        
            
                LEFT Join wsuber.bronze.map_ride_statuses map_ride_statuses on stg_rides.ride_status_id = map_ride_statuses.ride_status_id
            
        
            
                LEFT Join wsuber.bronze.map_payment_methods map_payment_methods on stg_rides.payment_method_id = map_payment_methods.payment_method_id
            
        
            
                LEFT Join wsuber.bronze.map_cities map_cities on stg_rides.pickup_city_id = map_cities.city_id
            
        
            
                LEFT Join wsuber.bronze.map_cancellation_reasons map_cancellation_reasons on stg_rides.cancellation_reason_id = map_cancellation_reasons.cancellation_reason_id
